...
object L {
    ...
    val playerDoesNotExist
        get() = "$prefix${get("$path.player_is_null")}"
    ...
    fun manageHelp(sender: CommandSender) =
            config.getStringList("$path.help.eco_command")
    ...
    val balanceTopHeader
        get() = "$prefix${get("$path.balance_top.header")}"

    val balanceTopEmpty
        get() = "$prefix${get("$path.balance_top.empty")}"

    val balanceTopNext
        get() = "$prefix${get("$path.balance_top.next")}"

    val balanceTop
        get() = "$prefix${get("$path.balance_top.balance")}"
    ...
    val balanceTopNoSupport
        get() = "$prefix${get("$path.balance_top.noSupport")}"
    ...
    object CurrencyUsage {
        ...
        fun sendUsage(sender: CommandSender) =
                config.getStringList("$path.help.currency_command")
                        .forEach { sender.sendMessage(StringUtils.colorize(it.replace("{prefix}", prefix))) }
    }
    ...
    val exchangeRateSet
        get() = "$prefix${get("$path.exchange_rate_set")}"
